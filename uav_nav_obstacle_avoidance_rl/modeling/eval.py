...
def evaluate_agent(
    ...
    """
    logger.info(
        f"Evaluating agent with custom metrics for {n_eval_episodes} episodes..."
    )
...
def compare_agents(
    ...
    for model_path in model_paths:
        ...
        logger.info(
            f"  Path Efficiency: {summary_metrics['avg_path_efficiency_ratio']:.3f}"
        )
...
def main(
    ...
    if len(model_files) > 1:
        ...
        logger.warning(
            "less than 2 models found for comparison -> running single evaluation."
        )
