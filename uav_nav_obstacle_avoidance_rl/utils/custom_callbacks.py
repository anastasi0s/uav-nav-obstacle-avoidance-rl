...
class TrainMetricsCallback(BaseCallback):
        ...
        try:
            ...
            for env_idx, env in enumerate(envs):
                ...
                while hasattr(underlying_env, 'env'):
                    if hasattr(underlying_env, 'waypoints') and hasattr(underlying_env, 'compute_attitude'):  # if base env has been found then stop peeling
                        break
                ...
                if self.current_episode_data[env_idx]['start_position'] is None:
                    self.current_episode_data[env_idx]['start_position'] = lin_pos.copy()

                # get target position
                self.current_episode_data[env_idx]['target_position'] = underlying_env.waypoints.targets[0].copy()  #NOTE adjust to multiple waypoints
    ...
    def _process_completed_episode(self, info: Dict, env_idx: int = 0):
        ...
        try:
            ...
            path_length = self._calculate_path_length(self.current_episode_data[env_idx]['positions'])
            mean_velocity = self._calculate_average_velocity(self.current_episode_data[env_idx]['velocities'])
    ...
    def _calculate_path_efficiency(self, start_position, target_position, path_length: float) -> float:                
        # direct distance to target
        direct_distance = np.linalg.norm(target_position - start_position)  # NOTE adjust this for multiple waypoint targets
...
class CustomEvalCallback(EvalCallback):
    ...
    def _log_eval_callback(self, locals_dict: dict, globals_dict: dict) -> None:
        ...
        try:
            ...
            if self.current_episode_data['target_position'] is None:
                self.current_episode_data['target_position'] = underlying_env.waypoints.targets[0].copy()   #TODO adjust to multiple waypoints
    ...
    def _process_completed_eval_episode(self, info: dict):
        ...
        try:
            ...
            # calculate custom metrics
            path_length = self._calculate_path_length(self.current_episode_data['positions'])
            mean_velocity = self._calculate_average_velocity(self.current_episode_data['velocities'])
            if success:
                # calculate path efficiency only on successful episodes where the target is reached 
                path_efficiency = self._calculate_path_efficiency(
            ...
            self.current_eval_cycle_data['positions_history'].append(np.array(self.current_episode_data['positions']).copy())
            self.current_eval_cycle_data['velocities_history'].append(np.array(self.current_episode_data['velocities']).copy())
            self.current_eval_cycle_data['target_position_history'].append(np.array(self.current_episode_data['target_position']).copy())  # NOTE adjust for mulitple waypoints
    ...
    def _log_after_evaluation(self):
        ...
        eval_metrics = {
            ...
            "eval_uav/out_of_bounds_rate": np.mean(self.current_eval_cycle_data['out_of_bounds']),
            ...
            "eval_uav/mean_velocity_mean": np.mean(self.current_eval_cycle_data['mean_velocities']),
    ...
    def _log_success_analysis(self):
        ...
        try:
            ...
            if success_df.empty or failure_df.empty:
                if self.verbose >= 1:
                    logger.warning("Insufficient data for success analysis (need both success and failure episodes)")
                return
            ...
            fig.add_trace(
                go.Histogram(x=success_df['path_lengths'], name='Success', 
                            marker_color='green', opacity=0.7, nbinsx=20),
                row=1, col=1
            )
            fig.add_trace(
                go.Histogram(x=failure_df['path_lengths'], name='Failure',
            ...
            fig.add_shape(
                type="line", x0=success_path_mean, y0=0, x1=success_path_mean, y1=1,
                line=dict(color="darkgreen", width=2, dash="dash"),
                xref="x", yref="y domain", row=1, col=1
            )
            fig.add_shape(
                type="line", x0=failure_path_mean, y0=0, x1=failure_path_mean, y1=1,
            ...
            fig.add_trace(
                go.Histogram(x=success_df['mean_velocities'], name='Success',
                            marker_color='green', opacity=0.7, nbinsx=20, showlegend=False),
                row=1, col=2
            )
            fig.add_trace(
                go.Histogram(x=failure_df['mean_velocities'], name='Failure',
                            marker_color='red', opacity=0.7, nbinsx=20, showlegend=False),
                row=1, col=2
            )
            ...
            fig.add_shape(
                type="line", x0=success_vel_mean, y0=0, x1=success_vel_mean, y1=1,
                line=dict(color="darkgreen", width=2, dash="dash"),
                xref="x2", yref="y2 domain", row=1, col=2
            )
            fig.add_shape(
                type="line", x0=failure_vel_mean, y0=0, x1=failure_vel_mean, y1=1,
                line=dict(color="darkred", width=2, dash="dash"),
                xref="x2", yref="y2 domain", row=1, col=2
            )
            ...
            fig.add_trace(
                go.Histogram(x=success_df['episode_rewards'], name='Success',
                            marker_color='green', opacity=0.7, nbinsx=20, showlegend=False),
                row=1, col=3
            )
            fig.add_trace(
                go.Histogram(x=failure_df['episode_rewards'], name='Failure',
                            marker_color='red', opacity=0.7, nbinsx=20, showlegend=False),
                row=1, col=3
            )
            ...
            fig.add_shape(
                type="line", x0=success_reward_mean, y0=0, x1=success_reward_mean, y1=1,
                line=dict(color="darkgreen", width=2, dash="dash"),
                xref="x3", yref="y3 domain", row=1, col=3
            )
            fig.add_shape(
                type="line", x0=failure_reward_mean, y0=0, x1=failure_reward_mean, y1=1,
                line=dict(color="darkred", width=2, dash="dash"),
                xref="x3", yref="y3 domain", row=1, col=3
            )
            ...
            fig.add_trace(
                go.Histogram(x=success_df['episode_lengths'], name='Success',
                            marker_color='green', opacity=0.7, nbinsx=20, showlegend=False),
                row=2, col=1
            )
            fig.add_trace(
                go.Histogram(x=failure_df['episode_lengths'], name='Failure',
                            marker_color='red', opacity=0.7, nbinsx=20, showlegend=False),
                row=2, col=1
            )
            ...
            fig.add_shape(
                type="line", x0=success_length_mean, y0=0, x1=success_length_mean, y1=1,
                line=dict(color="darkgreen", width=2, dash="dash"),
                xref="x4", yref="y4 domain", row=2, col=1
            )
            fig.add_shape(
                type="line", x0=failure_length_mean, y0=0, x1=failure_length_mean, y1=1,
                line=dict(color="darkred", width=2, dash="dash"),
                xref="x4", yref="y4 domain", row=2, col=1
            )
            
            # 5. Box plot comparison
            metrics_for_box = ['path_lengths', 'mean_velocities', 'episode_rewards', 'episode_lengths']
            for metric in metrics_for_box:
                fig.add_trace(
                    go.Box(y=success_df[metric], name=f'{metric} (Success)',
                          marker_color='lightgreen', showlegend=False),
                    row=2, col=2
                )
                fig.add_trace(
                    go.Box(y=failure_df[metric], name=f'{metric} (Failure)', 
            ...
            success_labels = ['Failure' if idx == 0 else 'Success' for idx in success_counts.index]
            success_colors = ['lightcoral' if idx == 0 else 'lightgreen' for idx in success_counts.index]
            
            if len(success_counts) > 0:
                fig.add_trace(
                    go.Pie(labels=success_labels, values=success_counts.values,
                          marker_colors=success_colors, name="Success Rate", showlegend=False),
                    row=2, col=3
                )
            ...
            failure_reasons = {
                ...
                'Timeout/Other': len(failure_df) - failure_df['collision'].sum() - failure_df['out_of_bounds'].sum()
            }
            ...
            if failure_reasons:
                fig.add_trace(
                    go.Pie(labels=list(failure_reasons.keys()), 
            ...
            fig.update_layout(
                height=1200,
                title_text="Success Analysis Dashboard",
                showlegend=True
            )
    ...
    def _log_trajectory_plot(self):
        ...
        try:
            ...
            picked_episodes = random.sample(range(len(episodes)), min(1, len(episodes)))  # pick 3 episode
            ...
            for idx, ep_idx in enumerate(picked_episodes):
                ...
                # Log to W&B
                wandb.log({
                    f"eval_trajectory/UAV_trajectory_{idx}": wandb.Html(fig.to_html(include_plotlyjs='cdn')),
                }, step=self.num_timesteps)
    ...
    def _log_correlation_matrix(self):
        ...
        try:
            ...
            df = df.drop(['positions_history', 'velocities_history', 'target_position_history'], axis=1)
